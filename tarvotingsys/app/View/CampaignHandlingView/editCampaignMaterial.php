...
<div class="container-fluid mt-4 mb-5">
  ...
  <div class="d-flex justify-content-between align-items-center mb-3">
    <h2 class="mb-0">Edit Campaign Material</h2>
  </div>
  ...
  <div class="card shadow-sm">
    <div class="card-body">
      ...
      <form action="/admin/campaign-material/edit/<?= (int)$campaignMaterial['materialsApplicationID'] ?>"
            method="POST"
            enctype="multipart/form-data"
            id="appForm"
            novalidate>

        <!-- Section: Election Event & Nominee (display only) -->
        ...
        <div class="mb-4">
          ...
          <div class="form-control-plaintext">
            <?= htmlspecialchars(
              ($campaignMaterial['nomineeFullName'] ?? '—') .
              ' (Nominee ID ' . (int)$campaignMaterial['nomineeID'] . ')'
            ) ?>
          </div>
        </div>

        <!-- Section: Campaign Materials Details -->
        <h5 class="mb-3">Campaign Materials Details</h5>

        <!-- Title -->
        <div class="mb-3">
          <label for="materialsTitle" class="form-label">
          ...
          <input type="text"
                 ...
                 id="materialsTitle"
                 name="materialsTitle"
                 value="<?= htmlspecialchars($campaignMaterial['materialsTitle'] ?? '') ?>"
                 required>
          <?php if (!empty($fieldErrors['materialsTitle'])): ?>
            <div class="invalid-feedback d-block">
              <?= htmlspecialchars($fieldErrors['materialsTitle'][0]) ?>
            </div>
          <?php endif; ?>
        </div>
        ...
        <!-- Type -->
        <div class="mb-3">
          <label for="materialsType" class="form-label">
            Type <span class="text-danger">*</span>
          </label>
          <select class="form-select<?= invalid($fieldErrors,'materialsType') ?>"
                  id="materialsType"
                  name="materialsType"
                  required>
            <?php
          ...
          <?php if (!empty($fieldErrors['materialsType'])): ?>
            <div class="invalid-feedback d-block">
              <?= htmlspecialchars($fieldErrors['materialsType'][0]) ?>
            </div>
          <?php endif; ?>
        </div>
        ...
        <!-- Description -->
        <div class="mb-3">
          <label for="materialsDesc" class="form-label">
            Description <span class="text-danger">*</span>
          </label>
          <textarea class="form-control<?= invalid($fieldErrors,'materialsDesc') ?>"
                    id="materialsDesc"
                    name="materialsDesc"
                    rows="4"
                    required><?= htmlspecialchars($campaignMaterial['materialsDesc'] ?? '') ?></textarea>
          <?php if (!empty($fieldErrors['materialsDesc'])): ?>
            <div class="invalid-feedback d-block">
              <?= htmlspecialchars($fieldErrors['materialsDesc'][0]) ?>
            </div>
          <?php endif; ?>
        </div>
        ...
        <!-- Quantity -->
        <div class="mb-4">
          <label for="materialsQuantity" class="form-label">
          ...
          <input type="number"
                 ...
                 min="1"
                 step="1"
                 value="<?= (int)($campaignMaterial['materialsQuantity'] ?? 1) ?>"
                 required>
          <?php if (!empty($fieldErrors['materialsQuantity'])): ?>
            <div class="invalid-feedback d-block">
              <?= htmlspecialchars($fieldErrors['materialsQuantity'][0]) ?>
            </div>
          <?php endif; ?>
        </div>
          ...
          <?php if (!empty($documents)): ?>
            <div class="list-group">
              <?php foreach ($documents as $d): ?>
                <?php
                  $fileUrl = '/uploads/campaign_material/' .
                             (int)$campaignMaterial['materialsApplicationID'] .
                             '/' . rawurlencode($d['materialsFilename']);
                ...
                <label class="list-group-item d-flex align-items-center justify-content-between">
                  ...
                  <span class="d-flex align-items-center gap-3">
                    ...
                    <div class="form-check m-0">
                      <input class="form-check-input cm-del"
                             type="checkbox"
                             value="<?= $docId ?>"
                             id="del<?= $docId ?>"
                             name="delete_docs[]">
                      <label class="form-check-label" for="del<?= $docId ?>">
                        Delete
                      </label>
                    </div>
                  </span>
        ...
        <div class="mb-4">
          ...
          <input type="file"
                 class="form-control<?= invalid($fieldErrors,'materialsFiles') ?>"
                 id="materialsFiles"
                 name="materialsFiles[]"
                 multiple
          ...
          <?php if (!empty($fieldErrors['materialsFiles'])): ?>
            <div class="invalid-feedback d-block">
              <?= htmlspecialchars($fieldErrors['materialsFiles'][0]) ?>
            </div>
          <?php endif; ?>
          <div class="form-text">
            You must have at least one file after deleting and adding.
          </div>
        </div>
