...
/**
 * expects:
 * $elections (list of ['electionID','title'] for this nominee)
...
// Logged-in nominee name for display
$nomineeName = $_SESSION['fullName'] ?? 'Nominee';
?>
...
<div class="container-fluid mt-4 mb-5">
  ...
  <div class="card shadow-sm">
    <div class="card-body">
      ...
      <form action="/nominee/campaign-material/create"
            method="POST"
            id="appForm"
            novalidate
            enctype="multipart/form-data">

        <!-- Section: Election & Nominee (same style as Create Schedule Location) -->
        <h5 class="mb-3">Election Event &amp; Nominee</h5>

        <!-- Election (searchable, like admin version) -->
        <div class="mb-3">
          ...
          <small class="text-muted">
            Only election events that you are eligible for will appear here.
          </small>
        </div>
        ...
        <div class="mb-3">
          ...
          <select name="materialsType"
            ...
            <option value="PHYSICAL" <?= (isset($old['materialsType']) && $old['materialsType'] === 'PHYSICAL') ? 'selected' : '' ?>>
              PHYSICAL
            </option>
            <option value="DIGITAL"  <?= (isset($old['materialsType']) && $old['materialsType'] === 'DIGITAL') ? 'selected' : '' ?>>
              DIGITAL
            </option>
          </select>
        ...
        <!-- Files (multiple; same chip design) -->
        <div class="mb-4">
        ...
        <!-- Actions (same alignment style) -->
        <div class="d-flex justify-content-end gap-2 pt-2">
          <a href="/nominee/campaign-material" class="btn btn-outline-secondary">
            Cancel
          </a>
          <a href="/nominee/campaign-material/create" class="btn btn-outline-secondary">
            Reset
          </a>
...
(function(){
  ...
  // Generic searchable dropdown (same behaviour as Create Schedule Location)
  ...
  // Multiple file input that keeps previous files (same as your original)
  ...
  // Final submit guard (same behaviour as Create Schedule Location for election)
  const form = $('appForm');
  if (form) {
