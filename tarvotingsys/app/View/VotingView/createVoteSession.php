...
<div class="container-fluid mt-4 mb-5">
  ...
  <form action="/vote-session/store" method="POST" id="createForm" novalidate>
    <!-- Session Details -->
    <fieldset class="fieldset mb-4">
      ...
      <div class="row g-3 align-items-start">
        <div class="col-md-6">
          ...
          <select name="electionID" class="form-select <?= !empty($fieldErrors['electionID']) ? 'is-invalid' : '' ?>"
            required>
        ...
        <div class="col-md-6">
          ...
          <input type="text" name="sessionName"
            class="form-control <?= !empty($fieldErrors['sessionName']) ? 'is-invalid' : '' ?>"
        ...
        <?php foreach ($old['races'] as $idx => $r): ?>
          <div class="race-card" data-index="<?= $idx ?>">
            ...
            <div class="mb-3">
              ...
              <input type="text" name="races[<?= $idx ?>][title]"
                class="form-control <?= race_err_has($raceRowErrors, $idx, 'title') ? 'is-invalid' : '' ?>"
                value="<?= htmlspecialchars($r['title'] ?? '') ?>" placeholder="e.g., Faculty Representative">
            ...
            <div class="row g-3 align-items-end">
              <div class="col-md-4">
                ...
                <select
                  class="form-select seat-type <?= race_err_has($raceRowErrors, $idx, 'seatType') ? 'is-invalid' : '' ?>"
                  name="races[<?= $idx ?>][seatType]">
                  <option value="">-- Select seat type --</option>
                  <option value="FACULTY_REP" <?= ($r['seatType'] ?? '') === 'FACULTY_REP' ? 'selected' : '' ?>>FACULTY_REP
                  </option>
                  <option value="CAMPUS_WIDE" <?= ($r['seatType'] ?? '') === 'CAMPUS_WIDE' ? 'selected' : '' ?>>CAMPUS_WIDE
                  </option>
                </select>
              ...
              <div class="col-md-4 faculty-col">
                ...
                <select
                  class="form-select faculty-select <?= race_err_has($raceRowErrors, $idx, 'facultyID') ? 'is-invalid' : '' ?>"
                  name="races[<?= $idx ?>][facultyID]">
                  <option value="">-- Select faculty --</option>
                  <?php foreach ($faculties as $f): ?>
                    <option value="<?= (int) $f['facultyID'] ?>" <?= (string) ($r['facultyID'] ?? '') === (string) $f['facultyID'] ? 'selected' : '' ?>>
              ...
              <div class="col-md-2">
                ...
                <input type="number" min="0" max="10"
                  ...
                  name="races[<?= $idx ?>][seatCount]" value="<?= isset($r['seatCount']) ? (int) $r['seatCount'] : 0 ?>">
            ...
            <div class="mt-3">
              ...
              <button type="button" class="btn btn-outline-danger btn-sm remove-race" <?= $onlyOne ? 'disabled aria-disabled="true" title="At least one race is required"' : '' ?>>
                Delete
              </button>
            </div>
          </div>
    ...
    <div class="d-flex justify-content-center gap-3">
      ...
      <button class="btn btn-primary" type="submit" name="publishMode" value="draft">Save as Draft</button>
      <button class="btn btn-success" type="submit" name="publishMode" value="schedule">Schedule</button>
    </div>
  ...
  (function () {
    ...
    function updateFacultyVisibility(card) {
      ...
      if (seatType === 'FACULTY_REP') {
        ...
        seatCount.value = 1; maxSel.value = 1;
        ...
        seatCount.value = 4; maxSel.value = 4;
        seatCount.setAttribute('readonly', 'readonly');
        maxSel.setAttribute('readonly', 'readonly');
        clampMaxSelectableToSeatCount(card, false);
      } else {
    ...
    document.getElementById('raceList').addEventListener('change', function (e) {
      ...
      if (e.target.classList.contains('seat-count') || e.target.classList.contains('max-select')) {
        ...
        if (seatType === '') {
          ...
          if (ms > sc) ms = sc;
          seatCount.value = sc; maxSel.value = ms;
        }
      }
    });
