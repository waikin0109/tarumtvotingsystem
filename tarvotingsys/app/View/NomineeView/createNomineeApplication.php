...
/**
 * expects:
 * $forms (list), $selectedForm (int), $renderAttrs (array),
 * $errors, $fieldErrors, $old, $students (list)
 */
...
<div class="container-fluid mt-4 mb-5">
    ...
    <div class="d-flex justify-content-between align-items-center mb-3">
        <h2 class="mb-0">Create Nominee Application</h2>
    </div>

    <div class="card shadow-sm">
        <div class="card-body">
            <form action="/admin/nominee-application/create"
                  method="POST"
                  id="appForm"
                  novalidate
                ...
                <div class="mb-3">
                    <label for="registrationFormID" class="form-label">
                        Registration Form <span class="text-danger">*</span>
                    </label>
                    <select
                        class="form-select<?= invalid($fieldErrors,'registrationFormID') ?>"
                    ...
                    <?php if (!empty($fieldErrors['registrationFormID'])): ?>
                        <div class="invalid-feedback d-block">
                            <?= htmlspecialchars(implode(' ', $fieldErrors['registrationFormID'])) ?>
                        </div>
                    <?php endif; ?>
                </div>
                ...
                <div class="mb-4">
                    ...
                    <?php if (!empty($fieldErrors['studentID'])): ?>
                        <div class="invalid-feedback d-block">
                            <?= htmlspecialchars(implode(' ', $fieldErrors['studentID'])) ?>
                        </div>
                    <?php endif; ?>

                    <div class="form-text">
                        Start typing the student’s name (or ID), then select from the list.
                    </div>
                </div>
                ...
                <div id="dynamic-fields">
                    ...
                    ?>
                        <div class="mb-3">
                            ...
                            <!-- File upload blocks tied to specific attributes -->
                            <?php if ($code === 'cgpa'): ?>
                                <div class="mt-2">
                                    <label class="form-label small">
                                        Upload CGPA proof (JPG/JPEG only) <span class="text-danger">*</span>
                                    </label>
                                    <input
                                        type="file"
                                        accept=".jpg,.jpeg"
                            ...
                            <?php elseif ($code === 'behaviorreport'): ?>
                                <div class="mt-2">
                                    <label class="form-label small">
                                        Upload behavior report (JPG/JPEG only) <span class="text-danger">*</span>
                                    </label>
                                    <input
                                        type="file"
                                        accept=".jpg,.jpeg"
                ...
                <div class="d-flex justify-content-end gap-2 pt-2">
                    ...
                    <a class="btn btn-outline-secondary" href="/admin/nominee-application/create">
                        Reset
                    </a>
                    <button type="submit" class="btn btn-primary">
                        Create Nominee Application
                    </button>
                </div>
            </form>
        </div>
...
document.getElementById('registrationFormID')?.addEventListener('change', function(){
    const id = this.value;
    if (!id) {
        window.location = '/admin/nominee-application/create';
        return;
    }
