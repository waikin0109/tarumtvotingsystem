...
<div class="container mt-4">
  ...
  <form action="/nominee-application/edit/<?= (int)$na['nomineeApplicationID'] ?>" method="POST" id="appForm" novalidate enctype="multipart/form-data">
    ...
    <div id="dynamic-fields">
      ...
      ?>
      <div class="mb-3">
        <label class="form-label" for="f-<?= htmlspecialchars($code) ?>"><?= htmlspecialchars($label) ?></label>
        ...
        <?php elseif ($type === 'checkbox'): ?>
          <div class="form-check">
            <input class="form-check-input<?= invalid($fieldErrors, $code) ?>" type="checkbox" id="f-<?= $code ?>"
                   name="fields[<?= $code ?>]" <?= ((int)$v===1) ? 'checked' : '' ?>>
            <label class="form-check-label" for="f-<?= $code ?>">Yes</label>
          </div>

        <?php else: ?>
          <input type="text" class="form-control<?= invalid($fieldErrors, $code) ?>"
                 id="f-<?= $code ?>" name="fields[<?= $code ?>]" value="<?= htmlspecialchars((string)$v) ?>">
        <?php endif; ?>

        <?php if ($code === 'cgpa'): ?>
          <div class="mt-2">
            <label class="form-label small">Upload CGPA proof (JPG/JPEG only) — optional (keeps old unless you delete below)</label>
        ...
        <?php elseif ($code === 'achievements'): ?>
          <div class="mt-2">
            <label class="form-label small">Upload additional achievement documents (JPG/JPEG only, multiple allowed)</label>
        ...
        <?php elseif ($code === 'behaviorreport'): ?>
          <div class="mt-2">
            <label class="form-label small">Upload behavior report (JPG/JPEG only) — optional</label>
            <input type="file" accept=".jpg,.jpeg"
    ...
    <div class="mb-4">
        ...
        <div class="list-group">
          <?php foreach ($documents as $doc): ?>
            <label class="list-group-item d-flex align-items-center justify-content-between">
              <span class="me-3"><?= htmlspecialchars($doc['academicFilename']) ?></span>
              <span class="d-flex align-items-center gap-3">
                ...
                <div class="form-check m-0">
                  <input class="form-check-input doc-del"
                         type="checkbox"
                         data-category="<?= htmlspecialchars(cat_from_filename($doc['academicFilename'])) ?>"
                         id="del-<?= (int)$doc['academicID'] ?>"
                         name="delete_docs[]" value="<?= (int)$doc['academicID'] ?>">
                  <label class="form-check-label" for="del-<?= (int)$doc['academicID'] ?>">Delete</label>
                </div>
              </span>
            </label>
          <?php endforeach; ?>
      ...
      <?php else: ?>
        <div class="form-control-plaintext text-muted">No documents uploaded yet.</div>
      <?php endif; ?>
    </div>
...
document.addEventListener('DOMContentLoaded', function(){
  ...
  function hasNewUpload(cat){
    if (cat === 'achievements') return (document.querySelector('input[name="uploads[achievements][]"]')?.files?.length||0) > 0;
    if (cat === 'cgpa') return (document.querySelector('input[name="uploads[cgpa]"]')?.files?.length||0) > 0;
    if (cat === 'behaviorreport') return (document.querySelector('input[name="uploads[behaviorreport]"]')?.files?.length||0) > 0;
    return false;
  }
